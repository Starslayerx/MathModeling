...
$\qquad \qquad N_n(x) = f(x_0) + (x-x_0)f[x_0,x_1]$
  
  $\qquad \qquad\qquad \qquad +\cdots+(x-x_0)(x-x_1)\cdots(x-x_n)f[x,x_0,x_1,\cdots,x_n]$,

  $\qquad \qquad R_n(x) =(x-x_0)(x-x_1)\cdots(x-x_n)f[x,x_0,x_1,\cdots,x_n]$


显然，$N_n(x)$是至多$n$次的多项式，称为牛顿插值多项式。$R_n(x)$称为牛顿插值余项

$\qquad$ 牛顿插值的优点是：每增加一个节点，插值多项式只增加一项，即
